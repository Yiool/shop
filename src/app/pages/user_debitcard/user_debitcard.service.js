...
(function () {
  ...
  angular
      .module('shopApp')
      .factory('userDebitcard', userDebitcard)
      .constant('userDebitcardConfig', userDebitcardConfig);

  /** @ngInject */
  function userDebitcard(requestService) {
    return requestService.request('users', 'debitcard');
  }


  /** @ngInject */
  function userDebitcardConfig() {
    var createtime = [
      {
        "label": "今日",
        id: 0,
      ...
      DEFAULT_GRID: {
        "columns": [
          ...
          },
          {
            "id": 8,
            "cssProperty": "state-column",
            "fieldDirective": '<a ui-sref="user.debitcard.detail({userid: item.map.userid, balance: item.map.balance, mobile: item.map.mobile})" class="u-btn-link">查看详情</a>',
            "name": '操作'
          }
        ],
        "config": {
      ...
      DEFAULT_GRID_DETAIL: {
        "columns": [
          ...
          },
          {
            "id": 2,
            "cssProperty": "state-column",
            "fieldDirective": '<span class="state-unread" bo-if="item.tradetype == 0">订单编号：<a bo-text="item.salesno?item.salesno:item.credentialsid" bo-if="item.tradetype == 6" ui-sref="user.package.list({page:1, keyword: item.salesno?item.salesno:item.credentialsid})"></a><a bo-text="item.salesno?item.salesno:item.credentialsid" ui-sref="trade.order.list({page:1, keyword: item.salesno?item.salesno:item.credentialsid})"></a></span><span class="state-unread" bo-if="item.tradetype == 2">储值卡购买</span>',
            "name": '交易内容'
          },
          {
            "id": 3,
